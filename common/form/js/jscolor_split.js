var colorPrePath = "/nms/Common/kit/form/";
var jscolor = {
	dir : "",
	bindClass : "color",
	binding : true,
	...
	URI : function(a) {
		...
		this.toAbsolute = function(e) {
			...
			if (d.scheme !== null
					&& d.scheme.toLowerCase() === e.scheme.toLowerCase()) {
				d.scheme = null
			}
			if (d.scheme !== null) {
				c.scheme = d.scheme;
	...
	color : function(A, d) {
		...
		this.showPicker = function() {
			if (!s()) {
				...
				var p = [
						2 * this.pickerBorder + 4 * this.pickerInset + 2
								* this.pickerFace + jscolor.images.pad[0] + 2
								* jscolor.images.arrow[0]
								+ jscolor.images.sld[0],
						2 * this.pickerBorder + 2 * this.pickerInset + 2
								* this.pickerFace + jscolor.images.pad[1] ];
				var K, I, H;
				...
				var F = (G[I] + p[I]) / 2;
				var E = [
						-D[K] + J[K] + p[K] > L[K] ? (-D[K] + J[K] + G[K] / 2 > L[K] / 2
								&& J[K] + G[K] - p[K] >= 0 ? J[K] + G[K] - p[K]
								: J[K])
								: J[K],
						-D[I] + J[I] + G[I] + p[I] - F + F * H > L[I] ? (-D[I]
								+ J[I] + G[I] / 2 > L[I] / 2
								&& J[I] + G[I] - F - F * H >= 0 ? J[I] + G[I]
								- F - F * H : J[I] + G[I] - F + F * H) : (J[I]
								+ G[I] - F + F * H >= 0 ? J[I] + G[I] - F + F
								* H : J[I] + G[I] - F - F * H) ];
				i(E[K], E[I])
			}
		};
		...
		this.exportColor = function(p) {
			...
			if (!(p & B) && C) {
				...
				C.style.color = 0.213 * this.rgb[0] + 0.715 * this.rgb[1]
		...
		this.fromHSV = function(F, E, D, p) {
			...
			E < 0 && (E = 0) || E > 1 && (E = 1);
			D < 0 && (D = 0) || D > 1 && (D = 1);
			this.rgb = g(F === null ? this.hsv[0] : (this.hsv[0] = F),
					E === null ? this.hsv[1] : (this.hsv[1] = E),
					D === null ? this.hsv[2] : (this.hsv[2] = D));
			this.exportColor(p)
		};
		this.fromRGB = function(G, F, p, D) {
			...
			F < 0 && (F = 0) || F > 1 && (F = 1);
			p < 0 && (p = 0) || p > 1 && (p = 1);
			var E = w(G === null ? this.rgb[0] : (this.rgb[0] = G),
					F === null ? this.rgb[1] : (this.rgb[1] = F),
					p === null ? this.rgb[2] : (this.rgb[2] = p));
			if (E[0] !== null) {
				this.hsv[0] = E[0]
			}
			if (E[2] !== 0) {
		...
		this.fromString = function(E, D) {
			...
			if (!p) {
				...
				if (p[1].length === 6) {
					this.fromRGB(parseInt(p[1].substr(0, 2), 16) / 255,
							parseInt(p[1].substr(2, 2), 16) / 255, parseInt(
									p[1].substr(4, 2), 16) / 255, D)
				} else {
					this
							.fromRGB(parseInt(p[1].charAt(0) + p[1].charAt(0),
									16) / 255, parseInt(p[1].charAt(1)
									+ p[1].charAt(1), 16) / 255, parseInt(p[1]
									.charAt(2)
									+ p[1].charAt(2), 16) / 255, D)
				}
				return true
			}
		};
		this.toString = function() {
			return ((256 | Math.round(255 * this.rgb[0])).toString(16)
					.substr(1)
					+ (256 | Math.round(255 * this.rgb[1])).toString(16)
		...
		function w(H, G, D) {
			...
			var F = H === I ? 3 + (D - G) / p : (G === I ? 5 + (H - D) / p : 1
					+ (G - H) / p);
		...
		function i(D, K) {
			...
			n = [ D + j.pickerBorder + j.pickerFace + j.pickerInset,
					K + j.pickerBorder + j.pickerFace + j.pickerInset ];
			...
			I.box.style.width = 4 * j.pickerInset + 2 * j.pickerFace
					+ jscolor.images.pad[0] + 2 * jscolor.images.arrow[0]
					+ jscolor.images.sld[0] + "px";
			I.box.style.height = 2 * j.pickerInset + 2 * j.pickerFace
			...
			I.padM.style.left = "0";
			I.padM.style.top = "0";
			I.padM.style.width = j.pickerFace + 2 * j.pickerInset
			...
			I.sldM.style.top = "0";
			I.sldM.style.width = jscolor.images.sld[0]
					+ jscolor.images.arrow[0] + j.pickerFace + 2
					* j.pickerInset + "px";
			...
			switch (b) {
			case 0:
				var H = colorPrePath + "skins/normal/form/hs.png";
				break;
			case 1:
				var H = colorPrePath + "skins/normal/form/hv.png";
				break
			}
			I.padM.style.background = "url(" + colorPrePath
					+ "skins/normal/form/cross.gif) no-repeat";
			I.sldM.style.background = "url(" + colorPrePath
					+ "skins/normal/form/arrow.gif) no-repeat";
			I.pad.style.background = "url('" + jscolor.getDir() + H
					+ "') 0 0 no-repeat";
			q();
			z();
		...
		function q() {
			...
			jscolor.picker.padM.style.backgroundPosition = (j.pickerFace
					+ j.pickerInset + J - Math
					.floor(jscolor.images.cross[0] / 2))
					+ "px "
					+ (j.pickerFace + j.pickerInset + I - Math
			...
			case 0:
				...
				for ( var D = 0; D < p.length; D += 1) {
					p[D].style.backgroundColor = "rgb("
							+ (H[0] * (1 - D / p.length) * 100) + "%,"
							+ (H[1] * (1 - D / p.length) * 100) + "%,"
							+ (H[2] * (1 - D / p.length) * 100) + "%)"
				}
				break;
			case 1:
				...
				for ( var D = 0; D < p.length; D += 1) {
					...
					G[1] = G[0] * (1 - K * E);
					G[2] = G[0] * (1 - K);
					p[D].style.backgroundColor = "rgb(" + (G[H[0]] * 100)
							+ "%," + (G[H[1]] * 100) + "%," + (G[H[2]] * 100)
							+ "%)"
				}
				break
		...
		function z() {
			...
			jscolor.picker.sldM.style.backgroundPosition = "0 "
					+ (j.pickerFace + j.pickerInset + D - Math
		...
		function u(D) {
			...
			switch (b) {
			case 0:
				j.fromHSV(p * (6 / (jscolor.images.pad[0] - 1)), 1 - E
						/ (jscolor.images.pad[1] - 1), null, e);
				break;
			case 1:
				j.fromHSV(p * (6 / (jscolor.images.pad[0] - 1)), null, 1 - E
						/ (jscolor.images.pad[1] - 1), e);
				break
			}
		}
		function h(p) {
		...
		var l = false;
		var a = jscolor.fetchElement(this.valueElement), C = jscolor
				.fetchElement(this.styleElement);
		var c = false, m = false;
		var n, x;
		var v = 1 << 0, B = 1 << 1, t = 1 << 2, e = 1 << 3;
		jscolor.addEvent(A, "focus", function() {
			if (j.pickerOnfocus) {
				j.showPicker()
			}
		});
