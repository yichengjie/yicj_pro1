define(function(require, exports, module) {
	...
	util.validFormData = function(formData ,orgFormData,NEW_ADD_STR){
		...
		var firstMaintenanceDate = util.getDate(formData['firstMaintenanceDate']);
		var statusDes = formData.statusDes ;
		if (statusDes != '3' && firstMaintenanceDate <= currDate) {
			$.showTuiErrorDialog('生效日期必须大于当前日期。');
			return false;
		}
		if(formData['lastMaintenanceDate'] !== '') {
			var lastMaintenanceDate = util.getDate(formData['lastMaintenanceDate']);
			if(lastMaintenanceDate < currDate){
				$.showTuiErrorDialog('截止日期必须大于等于当前日期。');
				return false;
			}
			if (firstMaintenanceDate > lastMaintenanceDate) {
				$.showTuiErrorDialog('截止日期必须大于等于生效日期。');
				return false;
			}
		}
		...
		// 费用相关校验
		if (formData['noChargeNotAvailable'] == '') {
			var trs = formData.list170VO ;
			if(trs.length==0){//这里判断依赖的是添加的一行记记录一定不能为空才这么简单的判断
				$.showTuiErrorDialog('当选择收费时，金额必须有值。');
				return false;
			}
		}else{//当为免费时清空表格
			var trs = formData.list170VO ;
		...
		if(serviceType=='C'||serviceType=='P'){//cp时后者填写不能大于前者
			var start = formData['firstExcessOccurrence']*1 ;
			var end = formData['lastExcessOccurrence']*1 ;
			if(start>end){
				$.showTuiErrorDialog('起始行李件数不能大于结束行李件数!');
				return false; 
			}
		}
		...
		if(formData['geoSpecFromToWithin']!=''){//如果不为不限区域则区域必填
			if(formData['geoSpecLoc1']==''){
				$.showTuiErrorDialog('当区域限制不为不限区域时，区域1的代码必须有值。');
				return false;
			}
		}
		if (formData['geoSpecFromToWithin'] == 'W' && (formData['geoSpecLoc2'] !== '' || formData['geoSpecLoc3'] !== '')) {
	...
	util.delInValidList = function(formData){
		...
		angular.forEach(formData.list183VO,function(m){
			if(util.strNotNull(m.geographicSpecificationType)){
				t183.push(m) ;
			}
		}) ;
